...
import FacturaFacade from './FacturaFacade';
...
const Impresion = () => {
  ...
  // Instancia el Facade pasando la URL base de la API
  const facturaFacade = new FacturaFacade("http://localhost:5000");
  //const urlDatos = "http://localhost:5000/impresionFactura/";

  useEffect(() => {
    const fetchDatosFactura = async () => {
      try {
        const datosFactura = await facturaFacade.obtenerDatosFactura(idFactura);
        setDatos(datosFactura);
      } catch (error) {
        alert("Error: " + error);
      }
    };
  ...
  const handlePrint = () => {
    facturaFacade.generarPDF({ idFactura }, "content-to-print");
  };
