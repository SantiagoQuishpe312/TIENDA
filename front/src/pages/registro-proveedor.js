...
const update = async (e) => {
  ...
  const ActivarProveedores= async (id) => {
    ...
    const renderProveedoresTabla = () => {
      ...
      const filteredProveedoresByUser = proveedores.filter((proveedor) => {
        const userRole = localStorage.getItem("role");
        if (userRole === "admin") {
          return proveedor.PROV_DIRECCION === "Quito"; // Filtra para el usuario "admin"
        } else if (userRole === "admin1") {
          return proveedor.PROV_DIRECCION !== "Quito"; // Filtra para el usuario "admin1"
        } else {
          return true; // Muestra todos los proveedores para otros usuarios
        }
      });
      
    // Separar proveedores con estado "Inactivo" y colocarlos al final de la lista
    const proveedoressActivos = filteredProveedoresByUser.filter(
      (proveedor) => proveedor.PROV_ESTADO === "Activo"
    );
    const proveedoressInactivos = filteredProveedoresByUser.filter(
    ...
    return (
      <table className="centrar-tabla">
        ...
        <tbody>
        {filteredProveedoresByUser.map((proveedor, index) => (
          <tr
            key={proveedor.PROV_ID} // Agregar una clave única aquí
            style={
...
import { useNavigate, useParams } from "react-router-dom";
import axios from "axios";
import ProveedorSingleton from "./ProveedorSingleton";
const URI = "http://localhost:5000/proveedores/";
...
const [selectedProveedorName, setSelectedProveedorName] = useState(""); // Estado para el nombre del proveedor seleccionado
    ...
    useEffect(() => {
      const auth = ProveedorSingleton.getInstance();
